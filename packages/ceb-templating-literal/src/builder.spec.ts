...
describe("builder", () => {
  ...
  it("should render into Light DOM", () => {
    ...
    ElementBuilder.get(TestElement).name(tagName).builder(TemplateBuilder.get()).register()
  ...
  it("should render into Grey DOM", () => {
    ...
    ElementBuilder.get(TestElement).name(tagName).builder(TemplateBuilder.get().grey()).register()
    sandbox.innerHTML = `<${tagName}>World</${tagName}>`
    const element = sandbox.querySelector(tagName) as TestElement
    assert.strictEqual(element.innerHTML.trim().replace(/[\s]/g, ""), `<p>Hello,<ceb-slot>World</ceb-slot>!</p>`)
  })
  ...
  it("should render into Shadow DOM", () => {
    ...
    ElementBuilder.get(TestElement).name(tagName).builder(TemplateBuilder.get().shadow()).register()
    const element = sandbox.appendChild(document.createElement(tagName) as TestElement)
    assert.ok(sandbox.querySelector(tagName))
