 ...
 * @private
 */
export type ContextItem = Element &
  HTMLElement &
  DocumentFragment & {
    /**
...
class Context {
  constructor(public readonly _element: ContextItem | null, private index: number = -1) {}
...
class Contexts {
  constructor(private readonly element: ContextItem, private readonly stack: Array<Context> = [new Context(element)]) {}
...
export class Engine {
  ...
  comment(text?: string): void {
    if (text) {
      this.handleNode(text, this.document.COMMENT_NODE, createCommentNode)
    }
  }
  ...
  text(text?: string): void {
    if (text) {
      this.handleNode(text, this.document.TEXT_NODE, createTextNode)
    }
  }
  ...
  private handleElement(name: string, parameters: Parameters = createDefaultParams()): ContextItem {
    ...
    if (referencedElement) {
      ...
      currentElement = this.contexts
        .get()
        .element.appendChild(createElement(this.document, name, parameters.attributes))
    }
