...
export interface IpcObservableEventBus extends ObservableEventBus {
   ...
   */
  on<K extends keyof IpcEventBusNotificationMap>(type: K, listener: (event: IpcEventBusNotificationMap[K]) => any): this
...
export interface IpcEmitterEventBus extends EmittableEventBus {
   ...
   * @template K the type of the internal event
   */
  emit<K extends keyof IpcEventBusNotificationMap>(type: K, event: IpcEventBusNotificationMap[K]): void
}
