...
export class IpcMainCommandBus implements CommandBus, Disposable {
    ...
    execute<R extends Result<any, ResultHeaders> = Result<any, ResultHeaders>, C extends Command<any, MessageHeaders> = Command<any, MessageHeaders>>(
        command: C, options?: Partial<ExecuteActionOptions>
    ): Promise<CommandResult<Result<any, ResultHeaders>>> {
    ...
    executeAndForget<C extends Command<any, MessageHeaders> = Command<any, MessageHeaders>>(
        command: C
    ): void {
        // forward to IPC
        webContents.getAllWebContents().forEach(webContent => webContent.send(
            IPC_CHANNEL_COMMANDS,
            command, {waitForResult: false})
        )
        // forward to parent
        try {
