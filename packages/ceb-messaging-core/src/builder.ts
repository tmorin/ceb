...
> {
   ...
   * @template M the type of the message
   */
  static get<B = any, H extends MessageHeaders = MessageHeaders, M extends Message<B, H> = Message<B, H>>(
    kind: MessageKind
  ) {
   ...
   * @template M the type of the message
   */
  static command<B = any, H extends MessageHeaders = MessageHeaders, M extends Command<B, H> = Command<B, H>>(
    type: string
  ) {
   ...
   * @template M the type of the message
   */
  static query<B = any, H extends MessageHeaders = MessageHeaders, M extends Query<B, H> = Query<B, H>>(type: string) {
   ...
   * @template M the type of the message
   */
  static result<B = any, H extends ResultHeaders = ResultHeaders, M extends Result<B, H> = Result<B, H>>(
    action: Action,
    type?: string
  ) {
    return new MessageBuilder<B, H, M>("result", type || "result").headers(<Partial<H>>{
   ...
   * @template M the type of the message
   */
  static empty<B = undefined, H extends ResultHeaders = ResultHeaders, M extends Result<B, H> = Result<B, H>>(
    action: Action
  ) {
   ...
   * @template M the type of the message
   */
  static event<B = any, H extends MessageHeaders = MessageHeaders, M extends Event<B, H> = Event<B, H>>(type: string) {
