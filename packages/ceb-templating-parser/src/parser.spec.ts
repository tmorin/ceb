...
describe("parser", function () {
  ...
  it("should parse elements", function () {
    const parseResult = executeParse(`<div><h1>title</h1><hr><fieldset><label>label</label><input/></fieldset></div>`)
    // div
    expect(parseResult[i].name).eq("openTag")
    expect(parseResult[i].detail.tagName).eq("div")
    i++
  ...
  it("should parse bulma card", function () {
    const parseResult = executeParse(
      `<div id="1"><div id="1.1"><div id="1.1.1">1.1.1</div></div><div id="1.2"><div id="1.2.1">1.2.1</div></div></div>`
    )
    // div id="1"
    expect(parseResult[i].name).eq("openTag")
    expect(parseResult[i].detail.tagName).eq("div")
