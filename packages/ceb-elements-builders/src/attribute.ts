 ...
 * The default kind of attribute is _String_, the switch to _Boolean_ can be done explicitly with {@link AttributeBuilder.boolean}
...
export class AttributeBuilder<E extends HTMLElement = HTMLElement> implements Builder<E> {
  ...
  decorate(prefix = "on"): MethodDecorator {
    return (target: Object, methName: string | symbol, descriptor: PropertyDescriptor) => {
      if (!this._attrName) {
        this._attrName = toKebabCase(methName.toString().replace(prefix, ""))
      }
