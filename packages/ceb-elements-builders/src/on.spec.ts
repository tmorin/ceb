...
import { getTagName, listen } from "@tmorin/ceb-elements-testing"
...
describe("on", () => {
  ...
  context("listen events on delegated elements", () => {
    ...
    beforeEach((done) => {
      ...
      ElementBuilder.get(OnBuilderListenEventsByDelegation)
        .before("connectedCallback", (el) => (el.innerHTML = '<i class="i1"><b>i1</b></i><i class="i2"><b>i2</b></i>'))
        .builder(
  ...
  context("skip propagation and default behavior", () => {
    ...
    beforeEach((done) => {
      ...
      ElementBuilder.get(OnBuilderSkipPropagationAndDefault)
        .before("connectedCallback", (el) => (el.innerHTML = "<button>button</button>"))
        .builder(OnBuilder.get("custom-event").skip().invoke(elListener))
        .register()
  ...
  context("listen global events", () => {
    ...
    let bubblingListener: SinonSpy, captureListener: SinonSpy, el: TestElement, tagName: string
    beforeEach((done) => {
      bubblingListener = sinon.spy()
      captureListener = sinon.spy()
      tagName = "on-global"
